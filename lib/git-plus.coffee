...
module.exports =
  config:
    ...
    splitPane:
      ...
      type: 'string'
      default: 'Down'
      description: 'Where should new panes go? (Defaults to Down)'
  ...
  activate: (state) ->
    ...
    if repos.length is 0
      @subscriptions.add atom.commands.add 'atom-workspace', 'git:init', -> GitInit()
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:menu', -> new GitPaletteView()
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:add', -> git.getRepo().then((repo) -> GitAdd(repo))
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:add-all', -> git.getRepo().then((repo) -> GitAdd(repo, addAll: true))
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:checkout-current-file', -> git.getRepo().then((repo) -> GitCheckoutCurrentFile(repo))
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:difftool', -> git.getRepo().then((repo) -> GitDifftool(repo))
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'git:pull-using-rebase', -> git.getRepo().then((repo) -> GitPull(repo, rebase: true))
    ...
    @subscriptions.add atom.config.observe 'git.syntaxHighlighting',
      (value) ->
